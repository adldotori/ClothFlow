...
def get_opt():  
    ...
    parser.add_argument("--stage1_model_pth", default = "stage1/checkpoints/tops/checkpoint_arm_5500.pth")
    parser.add_argument("--stage2_model_pth", default = "stage2/checkpoints/tops/checkpoint_1_2.pth")
...
def load_inputs(opt,name):
    ...
    path_cloth = osp.join(data_path,"cloth.jpg")
    path_image = osp.join(data_path,"image.png")
    path_cloth_mask = osp.join(data_path,"cloth_mask.jpg")
...
def main():
    ...
    for name in names:
        ...
        result = model3(cloth,off_cloth,pants,warped,cloth_mask,head,pose)
        ...
        img = img.cpu().clamp(0,255)
        img = img.detach().numpy().astype('uint8')
        Image.fromarray(img).save(osp.join(opt.result,name,"result.jpg"))
