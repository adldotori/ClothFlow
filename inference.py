...
def get_opt():
    ...
    parser.add_argument("--dataroot", default="/home/fashionteam/dataset_test")
    parser.add_argument("--mode", default = "one") # mode = all | one
    parser.add_argument("--name", default = "1") # valid if mode == one
    parser.add_argument("--version", default = "MVC") # version = MVC | viton
...
def main():
    ...
    #model1.to(device)
    model1 = nn.DataParallel(model1,output_device=2)
    model2 = M2.FlowNet(PYRAMID_HEIGHT,4,1)
    #device = torch.device("cuda:2")
    #model2.to(device)
    model2 = nn.DataParallel(model2,output_device=2)
    model3 = M3.UNet(opt,33,4)
    #device = torch.device("cuda:2")
    #model3.to(device)
    model3 = nn.DataParallel(model3,output_device=2)
    ...
    for name in names:
        ...
        #os.mkdir(osp.join(opt.result,name))
