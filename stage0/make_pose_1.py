...
IS_TOPS = False
H, W = (512, 512)
base_dir = "/home/fashionteam/pose_dataset/"
...
for i, folder in enumerate(folder_list):
    ...
    with open(os.path.join(base_dir, folder, 'pose.pkl'), 'rb') as f:
    ...
    for i in range(point_num):
        ...
        if c_pointx[i] > 1 and c_pointy[i] > 1 and (not IS_TOPS or i in [1,2,3,4,5,6,7,8,11]):
    ...
    if IS_TOPS:
        imsave(c_pose_map, os.path.join(base_dir, folder, 'pose_top.png'))
    else:
        imsave(c_pose_map, os.path.join(base_dir, folder, 'pose.png'))
